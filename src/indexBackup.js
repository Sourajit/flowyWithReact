...
export const load = ({props}) => {
    var { text, grab = () =>{},release = () =>{},snapping = () => true,rearrange = () => false, spacing_x = 20, spacing_y = 80} = props;
    var blocks = [];
    var blockstemp = [];
    ...
    const moveBlock = function(event) {
      var mouse_x, mouse_y;
      if (event.targetTouches) {
          mouse_x = event.targetTouches[0].clientX;
          mouse_y = event.targetTouches[0].clientY;
...
export class ReactFlowy extends PureComponent {
    constructor(props) {
      super(props);
      this.loaded = false;
    }
